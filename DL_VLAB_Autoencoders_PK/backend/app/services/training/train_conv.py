...
from app.services.models.conv_autoencoder import ConvAutoencoder
...
def train_conv(
    ...
    start = started_at()
    print(f"[Conv] Training on {device} for {epochs} epochs")

    for epoch in range(1, epochs + 1):
        model.train()
        running_train_loss = 0.0

        for images, _ in epoch_progress(train_loader, f"[Conv] Epoch {epoch}/{epochs} train"):
        ...
        with torch.no_grad():
            for images, _ in epoch_progress(val_loader, f"[Conv] Epoch {epoch}/{epochs} val"):
                images = images.to(device, non_blocking=True)
                reconstructed = model(images)
        ...
        torch.save(model.state_dict(), model_dir / "conv_autoencoder.pth")
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            torch.save(model.state_dict(), model_dir / "best_model.pth")

        print(
            f"[Conv] Epoch {epoch}/{epochs} | "
    ...
    print(f"[Conv] Done. Best val loss: {best_val_loss:.6f}. Time: {total_time:.2f}s")
    return history
