...
from app.services.models.sparse_autoencoder import SparseAutoencoder
...
def train_sparse(
    ...
    start = started_at()
    print(f"[Sparse] Training on {device} for {epochs} epochs")

    for epoch in range(1, epochs + 1):
        model.train()
        running_train_loss = 0.0

        for images, _ in epoch_progress(train_loader, f"[Sparse] Epoch {epoch}/{epochs} train"):
            ...
            sparsity_penalty = lambda_sparsity * torch.mean(torch.abs(latent))
        ...
        with torch.no_grad():
            for images, _ in epoch_progress(val_loader, f"[Sparse] Epoch {epoch}/{epochs} val"):
                images = images.to(device, non_blocking=True)
                latent = model.encode(images)
                reconstructed = model.decode(latent)
                ...
                mse_loss = criterion(reconstructed, images)
                sparsity_penalty = lambda_sparsity * torch.mean(torch.abs(latent))
                val_loss = mse_loss + sparsity_penalty
        ...
        torch.save(model.state_dict(), model_dir / "sparse_autoencoder.pth")
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            torch.save(model.state_dict(), model_dir / "best_model.pth")

        print(
            f"[Sparse] Epoch {epoch}/{epochs} | "
    ...
    print(f"[Sparse] Done. Best val loss: {best_val_loss:.6f}. Time: {total_time:.2f}s")
    return history
